...
import static funShop.security.SecurityConstants.HEADER_STRING;
import static funShop.security.SecurityConstants.TOKEN_PREFIX;

// triển khai một filter dùng để xác nhận token. 
//Filter OncePerRequestFilter là một filter được kích hoạt cho mỗi một lần request
	...
	protected void doFilterInternal(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse,
		...
		try {
			...
			if (StringUtils.hasText(jwt) && tokenProvider.validateToken(jwt)) {
				...
				User userDetails = customUserDetailsQueryService.loadUserById(userId);

				UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(
						userDetails, null, Collections.emptyList()); // NO credentials, NO role
